...
if ! tmux -L default attach-session -t ${SESSION}; then
  ...
  tmux new-session -d -s ${SESSION} -x $W -y $H \
    load-env-writing fish
  tmux set-option -t ${SESSION} default-command 'load-env-writing fish'
  ...
  tmux split-window -t "$SESSION:edit.0" -b -p 20 "load-env-writing ghcid *.hs"
